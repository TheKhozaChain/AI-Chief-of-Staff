...
| R033 | Multi-Day Consolidation Breakout (MDCB) | R021's success with ATR compression suggests longer consolidation periods may predict larger moves | 3-7 day sideways consolidation (measured by narrow Donchian channel relative to ATR) precedes explosive breakouts in the structural uptrend. | new | Archetype: range_compression_expansion. {'atr_period': 30, 'atr_lookback': 120, 'compression_percentile': 5, 'breakout_atr_mult': 2.0, 'confirmation_bars': 3, 'stop_pct': 2.5, 'target_pct': 8.0, 'long_only': True} |
| R034 | Momentum Divergence Acceleration (MDA) | R022 MERT shows RSI bounces work; acceleration_breakout archetype unused but promising | After RSI drops below 40 then recovers above 50 with price making higher lows, ROC acceleration confirms renewed bullish momentum worth riding. | new | Archetype: acceleration_breakout. {'lookback_period': 30, 'breakout_atr_mult': 1.0, 'roc_period': 5, 'roc_threshold': 2.0, 'stop_pct': 2.5, 'target_pct': 6.0, 'long_only': True} |
| R035 | Trending Strength Continuation (TSC) | R006 trend following worked (PF 1.26); unused trend_strength_breakout archetype adds ADX filter | Donchian breakouts above rising 50MA with ADX>25 indicate established trends likely to continue, filtering out weak/choppy periods. | new | Archetype: trend_strength_breakout. {'lookback_period': 20, 'breakout_atr_mult': 1.5, 'adx_period': 14, 'adx_threshold': 25, 'trend_ma_period': 50, 'stop_pct': 3.0, 'target_pct': 7.0, 'long_only': True} |
| R036 | Momentum Surge After Consolidation (MSAC) | Observing that R021 (RCE) works brilliantly with compression + breakout, and learning that "squeeze predicts magnitude not direction" - combining compression detection with directional momentum filter | After volatility compression, the FIRST strong directional momentum surge (3+ consecutive bars moving same direction) signals trend continuation rather than reversal, capturing explosive moves that follow tight ranges. | new | Archetype: consecutive_momentum. {'lookback': 20, 'breakout_atr_mult': 1.5, 'min_consecutive': 3, 'stop_pct': 2.5, 'target_pct': 6.0, 'atr_compression_percentile': 15, 'require_compression': True, 'long_only': True} |
| R037 | Acceleration Breakout With Volume Confirmation (ABVC) | R006 showed trend following works (PF 1.26) and our learnings note "Donchian breakout works (PF 1.31)" - combining price acceleration with volume surge should filter for high-conviction breakouts | Donchian breakouts that coincide with both ROC acceleration AND volume spikes (2x+ average) represent institutional accumulation points where large players are pushing price, creating sustainable trends rather than false breaks. | new | Archetype: volume_breakout. {'lookback': 30, 'volume_mult': 2.0, 'roc_period': 10, 'roc_threshold': 2.0, 'stop_pct': 2.5, 'target_pct': 5.0, 'require_roc': True, 'long_only': True} |
| R038 | Adaptive Volatility Regime Trend (AVRT) | R006 vol-adjusted strategies showed promise (PF 1.46 with ATR-based stops) and daily timeframe showed strong results - using ATR percentile regimes to scale position timing in trends | In high ATR percentile environments (volatile regimes), wider Donchian breakout parameters capture sustained moves while avoiding chop, whereas tight ranges require tighter breakouts - regime-adaptive entry timing improves edge in crypto's shifting volatility. | new | Archetype: atr_regime_adaptive. {'atr_period': 20, 'atr_lookback': 100, 'high_vol_percentile': 70, 'low_vol_percentile': 30, 'high_vol_lookback': 40, 'low_vol_lookback': 15, 'stop_pct': 2.5, 'target_pct': 6.0, 'long_only': True} |
